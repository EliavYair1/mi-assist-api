...
async def send_message(
    ...
    if body.image_base64 and body.image_type:
        openai_messages.append({"role": "user", "content": [{"type": "image_url", "image_url": {"url": f"data:{body.image_type};base64,{body.image_base64}"}}, {"type": "text", "text": body.message or "Please analyze this image from a safety and inspection perspective."}]})
    else:
...
async def get_conversations(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(Conversation).where(Conversation.user_id == current_user.id).order_by(Conversation.last_message_at.desc()).limit(20))
    convs = result.scalars().all()
    return [{"id": str(c.id), "title": c.title or "Untitled", "created_at": c.created_at.isoformat() if c.created_at else None, "last_message_at": c.last_message_at.isoformat() if c.last_message_at else None} for c in convs]


@router.get("/conversations/{conversation_id}/messages")
async def get_conversation_messages(conversation_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    from app.models import Conversation
    ...
    msgs_result = await db.execute(select(Message).where(Message.conversation_id == conversation_id).order_by(Message.created_at.asc()))
...
async def analyze_pdf(
    ...
    await usage_svc.increment_message_count(db, current_user.id)
    await db.commit()
    return {"reply": reply, "conversation_id": str(conversation.id), "filename": file.filename, "pages": len(reader.pages)}